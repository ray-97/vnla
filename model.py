...
class AttentionSeq2SeqModel(nn.Module):
    ...
    def __init__(self, vocab_size, hparams, device):
        ...
        enc_hidden_size = hparams.hidden_size // 2 \
            if hparams.bidirectional else hparams.hidden_size # half encoding size if bidirectional?
        self.encoder = EncoderLSTM(vocab_size,
                              ...
                              num_layers=hparams.num_lstm_layers) # encodes end goal (a sequence of words)
        ...
        self.decoder = AskAttnDecoderLSTM(hparams, agent_class, device) # decodes a sequence of actions, runs 2 forward passes to compute final navigation dist
