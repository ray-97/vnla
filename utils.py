...
def load_nav_graphs(scan, path=None):
    ...
    with open(os.path.join(DATA_DIR, 'connectivity/%s_connectivity.json' % scan)) as f:
        # connectivity dir under data dir, scan tells which connectivity.json to open
        G = nx.Graph()
        positions = {}
        data = json.load(f)
...
def load_datasets(splits, path, prefix=''):
    ...
    for split in splits:
        with open(os.path.join(path, prefix + '_%s.json' % split)) as f:
            data += json.load(f) # data is a string version of json data in the same format.
    return data
...
# Files are found in Yuxuan's directory. Had issues with fully copying them to my directory.
def load_panos_to_region(house_id, region_label_to_name, include_region_id = False):
    pano_file = '../../../../../yuxuan/matterport3d/v1/scans/' + house_id + \
