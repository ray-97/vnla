...
class ShortestPathOracle(object):
    ...
    def _compute_shortest_paths(self, scan, path=None):
        ...
        paths = dict(nx.all_pairs_dijkstra_path(graph)) # paths from graph of scan
        distances = dict(nx.all_pairs_dijkstra_path_length(graph)) # distances for respective paths
    ...
    def _find_nearest_point(self, scan, start_point, end_points):
        ...
        for end_point in end_points:
            d = self.distances[scan][start_point][end_point] # just assume this gives distance for a path
            if d < best_d:
                best_d = d
                best_point = end_point
        return best_d, best_point # returns nearest end point
    ...
    def _find_nearest_point_on_a_path(self, scan, current_point, start_point, goal_point):
        ...
        # get a set of points (in path), then find the nearest point from current_point
    ...
    def _shortest_path_action(self, ob):
        ...
        # Can we see the next viewpoint? No for now
...
class AskOracle(object):  ####CHANGE HERE####
    ...
    def _should_ask_rule_a_e(self, ob, nav_oracle=None):
        ...
        if ob['queries_unused'] <= 0:
            return self.agent_ask_actions.index('dont_ask'), 'exceed' # agent_ask_actions is same as question pool in ask_agent

        scan = ob['scan']
        current_point = ob['viewpoint']
        # nearest dist and nearest points
        _, goal_point = nav_oracle._find_nearest_point(scan, current_point, ob['goal_viewpoints'])

        agent_decision = int(np.argmax(ob['nav_dist'])) # nav_dist from softmax comes from _populate_agent_state_to_obs during rollout
        ...
        start_point = ob['init_viewpoint'] # start point vs current point?
        d, _ = nav_oracle._find_nearest_point_on_a_path(scan, current_point, start_point, goal_point)
        # gets a point along start and goal, that is closest to current. Also gets the respective distance
        if d > self.deviate_threshold: # if distance to closest point on the path deviates too much
    ...
    def _should_ask(self, ob, nav_oracle=None): # returns questions to be asked and reason for it
        # print to check experiment settings?

        if self.rule_a_e: # check from hparams, not included in json
    ...
    def __call__(self, obs, nav_oracle):
        should_ask_fn = functools.partial(self._should_ask, nav_oracle=nav_oracle) # nav_oracle is just used to find shortest dist
        actions, reasons = zip(*list(map(should_ask_fn, obs))) # apply should_ask_fn on each item of obs, each producing an action(ask) and reason
        actions = list(map(self._add_ignore, actions, obs)) # _add_ignore uses 2 arguments, actions and obs
        # print("actions:")
        # print(actions)
        # print("reasons:")
        # print(reasons)
...
class NextOptimalOracle(object):
    ...
    def __init__(self, hparams, agent_nav_actions, env_nav_actions,
        ...
        self.ask_oracle = make_oracle('ask', hparams, agent_ask_actions) # ask_oracle
    ...
    def add_scans(self, scans): # called during setup before first __call__
        self.nav_oracle.add_scans(scans)

    def next_ask(self, obs):
        return self.ask_oracle(obs, self.nav_oracle) # __call__ of ask_oracle, pass in obs and shortest path nav oracle

    def next_nav(self, obs):
        return self.nav_oracle(obs) # shortest path oracle's call
...
class StepByStepSubgoalOracle(object):
    ...
    # actions
    def _answer_question(self, actions, ob, q): # return strings
        scan = ob['scan']
        ...
        if self.agent_ask_actions[q] == 'arrive': # depends on q
    ...
    def _map_actions_to_instruction_hard(self, actions): # move based on shortest path to goal, this is a calculated cheat
        agg_actions = []
        cnt = 1
    ...
    def __call__(self, ob, q=None):
        action_seq = self.nav_oracle(ob) # shortest path oracle, maps obs to shortest path actions?
        if self.mode == 'qa':   # action_seq is a seq of actions to the nearest goal for 1 ob
            assert(q is not None)
            verbal_instruction, edit_type = self._answer_question(action_seq, ob, q) # q is the reason for asking
        elif self.mode == 'easy':
...
def make_oracle(oracle_type, *args, **kwargs):
    ...
    if oracle_type == 'direct':
        ...
        return MultistepShortestPathOracle(*args, **kwargs) # used in step by step subgoal oracle
    if oracle_type == 'verbal':
        # returns (the next n_step of optimal agent actions, verbal instruction in string), NOT in batch
        return StepByStepSubgoalOracle(*args, **kwargs) # advisor
    if oracle_type == 'verbal_v2':
        return QAOracleV2(*args, **kwargs) # advisor
