...
def set_path():
    ...
    hparams.load_path = hparams.load_path if hasattr(hparams, 'load_path') and \
        ...
        # use export directory and model prefix to come up with path for checkpoint file, eg. main_learned_nav_...ckpt
    ...
    # print("hparams.exp_dir: " + hparams.exp_dir)
    # print("hparams.load_path: " + hparams.load_path)
    # print("DATA_DIR: " + DATA_DIR)
    # print("hparams.data_path: " + hparams.data_path)
    # print("Paused to show paths when set_path() is called...")
    # input()
...
def load(path, device):
    ...
    # print(ckpt)
    # input()
    hparams = ckpt['hparams'] # ckpt object has hparams key
...
def train(train_env, val_envs, agent, model, optimizer, start_iter, end_iter,
    ...
    train_feedback = { 'nav' : hparams.nav_feedback, 'ask' : hparams.ask_feedback } # "sample", "teacher"
    ...
    for idx in range(start_iter, end_iter, hparams.log_every):
        ...
        if eval_mode:
            loss_str = '\n * eval mode'
        else: # train_env is VNLABatch..
            traj = agent.train(train_env, optimizer, interval, train_feedback) # see ask_agent for train. 
            # we are extending from verbal_ask_agent, but train method is inheritted from ask_agent
            
            # compute stats
            train_losses = np.array(agent.losses) # to be done after k actions, budget and qns impl, this is related to metrics
            ...
            # compute_ask_stats from what is returned from the training
        ...
        for env_name, (env, evaluator) in val_envs.items():
            ...
            if eval_mode:
                agent.results_path = hparams.path.replace('ckpt', '') + env_name + '.json'
            ...
            # compute_ask_stats from evaluation of agent on data
            loss_str += compute_ask_stats(traj, agent)
            
            # choose whether to save checkpoint if in training mode
...
if __name__ == "__main__":
    ...
    # Read configuration from a json file, eg. verbal_hard.json
    ...
    with torch.cuda.device(hparams.device_id):
        # Multi-seed only for evaluation (from command line arg?)
