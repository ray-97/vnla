...
class AskAgent(BaseAgent):
    ...
    def __init__(self, model, hparams, device, should_make_advisor=True):
        ...
        # depends on arg in bash command, how an agent ask qns
        self.random_ask = hparams.random_ask if hasattr(hparams, 'random_ask') else 0 # random
        self.ask_first = hparams.ask_first if hasattr(hparams, 'ask_first') else 0 # first
        self.teacher_ask = hparams.teacher_ask if hasattr(hparams, 'teacher_ask') else 0 # teacher
        self.no_ask = hparams.no_ask if hasattr(hparams, 'no_ask') else 0 # none

        self.max_input_length = hparams.max_input_length
        self.n_subgoal_steps  = hparams.n_subgoal_steps # is this k?
    ...
    @staticmethod
    def n_output_nav_actions(): # number of types of output navigation actions, excluding 2 - start and end?
    ...
    def _make_batch(self, obs):
        ...
        seq_tensor = np.array([self.env.encode(ob['instruction']) for ob in obs]) # convert list of encoded obs['instruction']
        # inst is a string, so a tokenizer is used for encoding.
    ...
    def _next_action(self, name, logit, target, feedback):
        ...
        print("_next_action feedback: " + feedback)
        if feedback == 'teacher':
            return target
        if feedback == 'argmax': # during test
    ...
    def _populate_agent_state_to_obs(self, obs, *args):
        nav_softmax, queries_unused, traj, ended, time_step = args # *args is just syntactic sugar, used for unpacking
        nav_dist = nav_softmax.data.tolist() # check nav_softmax.data?
        for i, ob in enumerate(obs):
            ob['nav_dist'] = nav_dist[i] # each ob will be assigned its respective nav_dist
    ...
    def rollout(self):
        # Reset environment
        obs = self.env.reset(self.is_eval) # env is a VNLABatch object, double check if how budget is related to reset
        batch_size = len(obs)

        # Sample random ask positions
        if self.random_ask: # if command line says it's random
    ...
    def _setup(self, env, feedback):
        ...
        self.env = env
        self.teacher.add_scans(env.scans) # oracle
    ...
    def train(self, env, optimizer, n_iters, feedback):
        ...
        for iter in range(1, n_iters + 1):
            optimizer.zero_grad()
            traj = self.rollout() # where forward takes place?
            if n_iters - iter <= 10:
                last_traj.extend(traj)
            self.loss.backward() # from _compute_loss() called by rollout
            optimizer.step() # model's params was passed into optimizer in train_val()
